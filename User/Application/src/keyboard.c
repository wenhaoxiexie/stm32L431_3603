...
uint8_t keyboard_password_save(uint8_t *password,uint8_t len,uint8_t permision)
{
	...
	// 验证是否超过最大数
	if(get_cur_info->cur_keyboard_index>100)
	{
		set_win6170(32);
		return LOCK_MAX;
	}
...
uint8_t keyboard_password_del(uint8_t index,uint8_t permision)
{
	...
	for(i=0;i<KEYBOARD_NUM;i++)
	{
		...
		{
			if(key_board_flash[i].index==index)
			{
				memset(key_board_flash[index].data,0xff,sizeof(key_board_flash[i].data));
				key_board_flash[index].del_flag=1;
				key_board_flash[index].data_len=0;

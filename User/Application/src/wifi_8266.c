...
uint8_t wifi_upload_all_info(void)
{
	...
	for(upload_index=0;upload_index<WIFI_UPLOAD_NUM;upload_index++)
	{
		...
		{
			if(strstr(wifi_flash[upload_index].upload_info,"3001")!=NULL)
			{
...
//		wifi_get_mac(wifi_sys_info.wifi_mac);
	sprintf((char*)wifi_flash->upload_info,"%d%%%d%%%s%%",WARN_UPLOAD,TRY_ERR_WARN,lock_time);
	...
	do{
		...
		if(pItem!=NULL)
		{
			if(reBuff==NULL)
				return HAL_OK;
			pItem+=locLen;
			while((pItem[0]!=0x0d) && (pItem[1]!=0x0a)&&(locCoun<128))
			{
				*locBuff++ = *pItem++;
				locCoun++;
			}

...
// %80:24:28:33:89:12%3002%1%1%0%2019-08-22 9:33:23%#
static int parse_delete_info(uint8_t* cmd_string,cmd_item_wan* cmd_string_item)
{
		int i_num;
    int s_pos=0, i_pos=0;
    int c;

    for( i_num = 0; i_num < CMD_MAX_ITEM; ){
		char *cmd_string_i_num;

		if( i_num == 0 )
			cmd_string_i_num = cmd_string_item->cmd0;
		else if( i_num == 1 )
			cmd_string_i_num = cmd_string_item->cmd1;
		else if( i_num == 2 )
			cmd_string_i_num = cmd_string_item->cmd2;
		else if( i_num == 3 )
			cmd_string_i_num = cmd_string_item->cmd3;
		else if( i_num == 4 )
			cmd_string_i_num = cmd_string_item->cmd4;
		else if( i_num == 5 )
			cmd_string_i_num = cmd_string_item->cmd5;
		else if( i_num == 6 )
			cmd_string_i_num = cmd_string_item->cmd6;
    ...
    if( c== '%' ){
      ...
      cmd_string_i_num[i_pos] = '\0';
			return (i_pos? (i_num+1) : i_num);
    }
    else
      cmd_string_i_num[i_pos++] = c;
    }

    return i_num;
}

uint8_t app_delete_menu(void)
{
	char userList[8192]={0};
	uint8_t reStatus;
	uint8_t reBuff[128];
	...
	{
		debug_printf(" esp8266 build ssl \r\n");
		goto WAIT_FOR;
	}else{
		// 请先配置网络
		set_win6170(49);
	...
	// 本地用户列表同步
	if(strncmp((char*)wifi_sys_info.wifi_mac,"FFFFFFFFFFFF",6)==NULL)
...
WAIT_FOR:
	...
	delay_count = 60;
		...
		{
			// 解析数据
			parse_delete_info(reBuff,&cmdItem);
			debug_printf(">>>>>>>>>>>>>>>>>> %s ;%s ;%s ;%s ;%s ;%s ;%s \r\n",cmdItem.cmd0,cmdItem.cmd1,cmdItem.cmd2,
																															 cmdItem.cmd3,cmdItem.cmd4,cmdItem.cmd5,cmdItem.cmd6);
			if(atoi(cmdItem.cmd4)==FINGER_TYPE)
			{
				fp_delete(atoi(cmdItem.cmd3),atoi(cmdItem.cmd5));
				return LOCK_OK;
			}
			else if(atoi(cmdItem.cmd4)==KEYBOARD_TYPE)
			{
				keyboard_password_del(atoi(cmdItem.cmd3),atoi(cmdItem.cmd5));
				return LOCK_OK;
			}
			else if(atoi(cmdItem.cmd4)==CARD_TYPE)
			{
				nfc_delete(atoi(cmdItem.cmd3),atoi(cmdItem.cmd5));
				return LOCK_OK;
			}
			break;
		}
...
uint8_t sys_operator_log(void)
{
	...
	press_count=get_cur_info->cur_log_number-1;
	All_Screen();
	...
	do{
		if(get_cur_number.key_prees==1)
		{
			...
			else if(get_cur_number.number==8)
			{
				// 向下翻页
				press_count++;
				if(press_count>get_cur_info->cur_log_number-1)
					press_count=0;
				display_sys_log_menu(press_count);
			}
			else if(get_cur_number.number==10)
...
void sys_log_record(uint8_t index,uint8_t state,uint8_t permision,uint8_t type,uint8_t cur_index)
{
	...
	debug_printf("%02d:%02d:%02d\r\n",stimestructure.Hours, stimestructure.Minutes, stimestructure.Seconds);
	debug_printf("state :%d  cur_index:%d\r\n",state,cur_index);

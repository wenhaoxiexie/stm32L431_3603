...
void display_sys_source_info(uint8_t fp_num,uint8_t keyboard_num,uint8_t card_num)
{
	...
	if(keyboard_num<=9)
	{
		...
		for(i=2;i<6;i++)
		{
			display_one_word(&SOURCE_USED[i*16+128+64+96+64],8,16,4,i*8+4+32+8);
		}
	}
	// 感应卡
	for(i=0;i<3;i++)
...
void display_company_name(void)
{
	...
	for(i=0;i<8;i++)
	{
		for(j=0;j<128;j++)
		{
			OLED_GRAM[j][i]=company2[i*128+j];
		}
	}
}
...
void display_confirm(uint8_t type,uint8_t permision,uint8_t number)
{
	...
	else if(type==APP_TYPE)
	{
		for(j=0;j<3;j++)
		{
			display_one_word(&APP_OPEN[j*16],8,16,2,j*8+20);
		}
		for(j=0;j<4;j++)
		{
			display_one_word(&APP_OPEN[48+j*32],16,16,2,j*16+20+24);
		}
	}
	// 用户：
	for(i=0;i<3;i++)
...
void display_sys_log_menu(int16_t cur_index)
{
	...
	if(cur_index<LOG_NUM)
	{
		...
		display_one_word(&NUMBER[cur_index/10*16],8,16,0,2*8+28);
		...
		display_one_word(&NUMBER[(get_cur_info->cur_log_number)/10*16],8,16,0,6*8+28);
	...
	display_one_word(&NUMBER[log_flash[cur_index].index/10*16],8,16,2,2*8+32);
...
void display_delete_countdown(uint8_t count)
{
	uint8_t i;

	for(i=0;i<3;i++)
	{
		display_one_word(&DELETE_COUNTDOWN[i*32],16,16,1,i*16+40);
	}
